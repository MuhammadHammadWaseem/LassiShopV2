...
<div class="breadcrumb">
    <h1>{{ __('translate.Payments_Sale_Return') }}</h1>
</div>
        ...
        <div class="text-left">
            <div class="text-end bg-transparent mb-3">
                ...
                <div class="table-responsive">
                    <table id="payment_sale_return_table" class="display table table-hover">
                        <thead>
                            <tr>
         ...
         <!-- Modal Filter -->
         <div class="modal fade" id="filter_payment_sale_return_modal" tabindex="-1" role="dialog"
         aria-labelledby="filter_payment_sale_return_modal" aria-hidden="true">
         <div class="modal-dialog modal-lg" role="document">
             <div class="modal-content">
                 ...
                 <div class="modal-body">
                     ...
                     <form method="POST" id="filter_payment_sale_return_report">
                         @csrf
                         <div class="row">
                             ...
                             <div class="col-md-6">
                                 <label for="client_id" class="ul-form__label">{{ __('translate.Client') }}
                                 </label>
                             ...
                             <div class="col-md-6">
                                 <label for="sale_return_id"
                                     class="ul-form__label">{{ __('translate.Return') }} </label>
                                 <select name="sale_return_id" id="sale_return_id" class="form-control">
                                     <option value="0">{{ __('translate.All') }}</option>
                                     @foreach ($sale_returns as $sale_return)
                                     <option value="{{$sale_return->id}}">{{$sale_return->Ref}}</option>
                                     @endforeach
                                 </select>
                             </div>
...
<script type="text/javascript">
    $(function () {
      ...
      $(document).ready(function () {
        ...
        //init datatable
        payment_sale_return_datatable();

      });


        //Get Data
        function payment_sale_return_datatable(start_date ='', end_date ='', Ref ='', client_id ='', sale_return_id ='' , payment_method_id ='' , account_id =''){
            var $symbol_placement = @json($symbol_placement);
            var table = $('#payment_sale_return_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('payment_sale_return_report') }}",
                    data: {
                        ...
                        sale_return_id: sale_return_id == '0'?'':sale_return_id,
                        "_token": "{{ csrf_token()}}"
                    },
                },
                columns: [
                ...
                buttons: [
                    {
                        ...
                        //     },
                        //     title: function(){
                        //         return 'Report Payment Sell return';
                        //     },
                        //   },
                          {
                            extend: 'pdf',
        ...
        $('#Clear_Form').on('click' , function (e) {
            ...
            let sale_return_id = $('#sale_return_id').val('0');
        ...
        $('#Show_Modal_Filter').on('click' , function (e) {
            $('#filter_payment_sale_return_modal').modal('show');
        });


         // Submit Filter
        $('#filter_payment_sale_return_report').on('submit' , function (e) {
            e.preventDefault();
            ...
            let sale_return_id = $('#sale_return_id').val();

            $('#payment_sale_return_table').DataTable().destroy();
            payment_sale_return_datatable(start_date, end_date, Ref, client_id, sale_return_id , payment_method_id, account_id);

            $('#filter_payment_sale_return_modal').modal('hide');
...
<script>
    ...
    document.getElementById("printButton").addEventListener("click", function() {
          var table = document.getElementById("payment_sale_return_table");
          if (table) {
              // Clone the table
