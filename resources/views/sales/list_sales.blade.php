      ...
      <div class="card">
        <div class="card-body">
          <div class="text-end bg-transparent mb-3">
            @can('sales_add')
            <div class="col-md-4">
                <label for="attendent" class="form-label d-flex align-items-center">Select Attendent</label>
                <select class="form-select" name="attendent" id="attendent" aria-label="Default select example" onchange="changeAttendent()">
                    <option value="" disabled>Select Attendent</option>
                    @foreach ($users as $user)
                        <option value="{{ $user->name }}">{{ $user->username ?? 'N/A' }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="row">
              ...
              <div class="form-group col-md-6">
                ...
                <select name="client_id" id="client_id" class="form-control">
                  ...
                  <option value="{{$client->id}}" id="username">{{$client->username}}</option>
                  @endforeach
                </select>
              </div>
...
// Attendant counter work
function changeAttendent() {
            var selectElement = document.getElementById('attendent');
            ...
            searchInputs.forEach(function(input) {
                ...
                input.dispatchEvent(new Event('input')); // Trigger ('keyup');
            });
        }
  </script>
@endsection
