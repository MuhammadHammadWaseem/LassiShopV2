...
class ProductsController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($user_auth->can('products_add')) {
            $posCategory = Category::where('deleted_at', null)->where('is_ingredient', '=', 0)->get();
            $brands = Brand::where('deleted_at', null)->get(['id', 'name']);
            $categories = Category::where('deleted_at', null)->where('is_ingredient', '=', 1)->get(['id', 'name']);
            $warehouse = Warehouse::where('deleted_at', null)->get(['id', 'name']);
            $units = Unit::where('deleted_at', null)->where('base_unit', null)->get();

            return view('products.create_product', compact('categories', 'brands', 'units','warehouse','posCategory'));
        }
        return abort('403', __('You are not authorized'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($user_auth->can('products_add')) {
            ...
            \DB::transaction(function () use ($request) {
                ...
                // Auto create pos product
                if($request->no_ingredients === "true"){
                    $newProduct = new NewProduct();
                    $newProduct->name = $request['name'];
                    $newProduct->category_id = $request['pos_category'];
                    $newProduct->warehouse_id = $request['warehouse_id'];
                    // if ($request->hasFile('image')) {
                    //     $image = $request->file('image');
                    //     $filename = time() . '.' . $image->extension();
                    //     $newProduct->img_path = $filename;
                    // }
                    $newProduct->img_path = $filename;
                    $newProduct->price = $request['price'];
                    $newProduct->online_product_price = $request['online_price'];
                    $newProduct->save();

                    $newProductDetails = new NewProductDetail();
                    $newProductDetails->new_product_id = $newProduct->id;
                    $newProductDetails->base_product_id = $Product->id;
                    $newProductDetails->unit_id = $request['unit_sale_id'] ? $request['unit_sale_id'] : $request['unit_id'];
                    $newProductDetails->qty = 1;
    ...
    public function Products_by_Warehouse(request $request, $id)
    {
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if ($product_warehouse->product_variant_id) {
                ...
                $item['Variant'] = null;
                $item['code'] = $product_warehouse['product']->code ?? null;
    ...
    public function ProductByCategory(Request $request)
    {
        ...
        if ($request->type == 'Warehouse') {
            ...
            if ($request->category_id == 'all') {
                ...
                $products = NewProduct::where('warehouse_id', $request->warehouse_id)->where('category_id', $request->category_id)->get();
                return response()->json([
                    'products' => $products
                ]);
            }
        ...
        if ($request->type == 'Category') {
            if ($request->category_id == 'all') {
                ...
                $products = NewProduct::where('category_id', $request->category_id)->where('warehouse_id', $request->warehouse_id)->get();
                return response()->json([
                    'products' => $products
                ]);
            }
